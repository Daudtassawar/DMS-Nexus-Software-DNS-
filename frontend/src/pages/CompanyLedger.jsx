...
import { ArrowLeft, Plus, FileText, ArrowDownRight, ArrowUpRight, Edit2, Trash2, Download, Calendar, Undo, Landmark, RefreshCcw, Banknote, Calculator, History, CheckCircle, AlertCircle, Search, Filter } from 'lucide-react';
...
const CompanyLedger = () => {
  ...
  const [transaction, setTransaction] = useState({ id: null, transactionType: 'Credit', amount: '', description: '', reference: '', date: '' });
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const compRes = await axios.get(`/api/v1/Company/${id}`);
      setCompany(compRes.data);
      const ledRes = await axios.get(`/api/v1/CompanyLedger/${id}`);
  ...
  const handleTransaction = async (e) => {
    ...
    try {
      ...
      if (transaction.id) {
        await axios.put(`/api/v1/CompanyLedger/${transaction.id}`, payload);
        showStatus('success', 'Transaction record updated successfully.');
      } else {
        await axios.post('/api/v1/CompanyLedger', payload);
        showStatus('success', 'New transaction entry created.');
      }
      ...
      showStatus('error', `Entry rejected: ${msg}`);
    }
  };

  const resetTransactionForm = () => {
    setTransaction({ id: null, transactionType: 'Credit', amount: '', description: '', reference: '', date: '' });
  };
  ...
  const handleDeleteRequest = (tId) => {
    ...
    const timer = setTimeout(async () => {
      try {
        await axios.delete(`/api/v1/CompanyLedger/${tId}`);
        fetchData();
      } catch (err) {
        showStatus('error', 'Critical error: Transaction deletion failed.');
      } finally {
  ...
  const exportPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(18);
    doc.text(`${company?.name} - Statement of Account`, 14, 22);
    ...
    if (startDate || endDate) {
      doc.text(`Statement Period: ${startDate || 'All Time'} to ${endDate || 'Present'}`, 14, 36);
    }
    
    const tableColumn = ["Date", "Description", "Reference", "Debit (Add)", "Credit (Pay)", "Balance"];
    const tableRows = [];
  ...
  if (loading && !company) return (
    <div className="flex flex-col items-center justify-center min-h-[400px] animate-fade-in">
      ...
      <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Reconciling Ledger Data...</span>
    </div>
  );
    ...
    <div className="space-y-6 max-w-[1700px] mx-auto animate-fade-in pb-20">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 bg-[var(--bg-card)] p-6 rounded-lg border border-[var(--border)] shadow-sm">
        <div className="flex items-start gap-4">
          <button onClick={() => navigate('/companies')} className="mt-1 p-2 border border-slate-200 rounded-md text-slate-400 hover:bg-slate-50 hover:text-slate-900 transition-colors">
            <ArrowLeft size={18} />
          </button>
          <div>
            <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-3">
              <Landmark className="text-blue-500" size={24}/> {company?.name} <span className="text-slate-400 font-medium">Financial Statement</span>
            </h1>
            <p className="text-sm text-slate-500 mt-1">Audit transaction history, reconcile outstanding balances, and track vendor liabilities.</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <AppButton onClick={() => { resetTransactionForm(); setShowModal(true); }} className="rounded-md">
            <Plus size={18} className="mr-2"/> Record Entry
          </AppButton>
        </div>
      </div>
        ...
        <AppCard className="border border-slate-200">
           <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Statement Period</span>
        ...
        {pendingDeletes.map(pdId => (
          <div key={pdId} className="bg-slate-900 text-white px-6 py-4 rounded-lg shadow-2xl flex items-center gap-5 animate-slide-up border border-white/5">
            ...
            <div className="flex-1">
              <p className="text-xs font-bold uppercase tracking-tight">Entry flagged for deletion</p>
              <p className="text-[9px] text-slate-400 uppercase font-medium mt-0.5">Finalizing in 7 seconds...</p>
            </div>
      ...
      <AppCard p0 className="overflow-hidden shadow-sm border border-[var(--border)]">
        <div className="flex items-center justify-between p-4 bg-[var(--secondary)] border-b border-[var(--border)]">
           <div className="flex items-center gap-2 text-slate-600">
              <History size={16}/>
              <span className="text-xs font-bold uppercase tracking-wider">Transaction Registry</span>
           </div>
           <AppButton variant="secondary" size="sm" onClick={fetchData} className="rounded-md">
              <RefreshCcw size={14} className="mr-2"/> Sync Data
           </AppButton>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[var(--secondary)] border-b border-[var(--border)]">
                ...
                <th className="px-6 py-4 text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Description</th>
                <th className="px-6 py-4 text-[10px] font-bold text-red-500 uppercase tracking-widest text-right">Debit (+)</th>
                <th className="px-6 py-4 text-[10px] font-bold text-emerald-600 uppercase tracking-widest text-right">Credit (-)</th>
                <th className="px-6 py-4 text-[10px] font-bold text-[var(--text-main)] uppercase tracking-widest text-right">Net Balance</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredLedger.length === 0 ? (
                <tr>
                  <td colSpan="6" className="px-6 py-12 text-center text-slate-400 font-bold uppercase text-[10px] tracking-widest">No transaction records identified in current scope</td>
                </tr>
              ) : (
        ...
        <div className="fixed inset-0 bg-black/70 z-[1000] flex items-center justify-center p-6 animate-fade-in">
          <AppCard className="w-full max-w-xl border border-[var(--border)] shadow-md relative">
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                <h2 className="text-xl font-bold text-[var(--text-main)]">{transaction.id ? 'Modify Record' : 'New Transaction Record'}</h2>
                <p className="text-xs text-[var(--text-muted)] mt-1">Manual adjustment of principal entity ledger balance.</p>
              </div>
            </div>
            ...
            <form onSubmit={handleTransaction} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Entry Classification</label>
                  <select 
                  ...
                  >
                    <option value="Credit">CREDIT (Increase Liability)</option>
                    <option value="Debit">DEBIT (Decrease Liability)</option>
                  </select>
                </div>
                <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Transaction Specification</label>
                <AppInput 
                  placeholder="Provide detailed nature of this entry..." 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Audit Reference</label>
                  <AppInput 
                    placeholder="Invoice #, Voucher #, etc." 
                ...
                {transaction.id && (
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Temporal Adjustment</label>
                    <input 
                      type="datetime-local" 
              ...
              <div className="flex gap-4 pt-4">
                <AppButton variant="secondary" type="button" onClick={() => setShowModal(false)} className="flex-1 rounded-md">Cancel</AppButton>
                <AppButton type="submit" className="flex-1 rounded-md">Execute Record Entry</AppButton>
              </div>
            </form>
          </AppCard>
        </div>
